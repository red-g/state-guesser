...
-- This is a parsing script to format the SVGs from the svg format to elm


logParsedStates : () -> String
logParsedStates _ =
    case run parser stateSvgs of
        Ok s ->
            logUnescaped <| toString s

        Err _ ->
            Debug.todo "Error parsing statesSvgs"


logUnescaped : String -> String
logUnescaped s =
    Debug.log s ""
...
toString : List State -> String
toString states =
    "[" ++ (String.join "," <| List.map stateToString states) ++ "]"


stateToString : State -> String
stateToString ( name, props ) =
    "(" ++ name ++ "," ++ "stateWith { class=\"" ++ props.class ++ "\", d=\"" ++ props.d ++ "\", title=\"" ++ props.title ++ "\" })"


type alias State =
    ( String, { class : String, d : String, title : String } )


parser : Parser (List State)
parser =
    loop [] parserHelper


parserHelper : List State -> Parser (Step (List State) (List State))
parserHelper states =
    oneOf
        [ succeed (\_ -> Done <| List.reverse states)
            |= end
        , succeed (\item -> Loop <| item :: states)
            |. whiteSpace
            |= stateParser
        ]
...
stateParser =
    succeed (\{ properties, inner } -> ( inner, { class = properties.class, d = properties.d, title = inner } ))
        |= tags "path" pathAttrParser titleParser


pathAttrParser : Parser { class : String, d : String }
pathAttrParser =
    succeed (\class d -> { class = class, d = d })
        |. whiteSpace
        |= property "class"
        |. whiteSpace
        |= property "d"
...
titleParser =
    succeed identity
        |. whiteSpace
        |= map .inner (tags "title" (succeed ()) <| getChompedString <| chompUntil "<")


property : String -> Parser String
property name =
    succeed identity
        |. symbol name
        |. symbol "=\""
...
tags : String -> Parser a -> Parser b -> Parser { properties : a, inner : b }
tags name properties inner =
    succeed (\p i -> { properties = p, inner = i })
        |. symbol "<"
        |. symbol name
        |= properties
        |. symbol ">"
...
bordersParser : Parser (List Border)
bordersParser =
    loop [] bordersParserHelper


bordersParserHelper : List Border -> Parser (Step (List Border) (List Border))
bordersParserHelper borders =
    oneOf
        [ succeed (\_ -> Done <| List.reverse borders)
            |= end
        , succeed (\item -> Loop <| item :: borders)
            |. whiteSpace
            |= borderParser
        ]
...
borderParser =
    closedTag "path" pathAttrParser


closedTag : String -> Parser a -> Parser a
closedTag name properties =
    succeed identity
        |. symbol "<"
        |. symbol name
        |= properties
...
borderToString border =
    "borderWith { class = \"" ++ border.class ++ "\", d = \"" ++ border.d ++ "\" }"


bordersToString : List Border -> String
bordersToString borders =
    "[" ++ (String.join "," <| List.map borderToString borders) ++ "]"


logParsedBorders : () -> String
logParsedBorders _ =
    case run bordersParser borderSvgs of
        Ok borders ->
            logUnescaped <| bordersToString borders
